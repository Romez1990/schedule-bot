...
from structures import Schedule, Group, GroupSchedule, WeekDay, \
    DaySchedule, ScheduleKind
...
class Renderer:
    ...
    def _create_image(self, schedule: Schedule) -> None:
        ...
        image_size = (
            ...
            self._separator_width *
            (len(self._schedule_metrics.week_days) - 1),
        )
        self._image = Image(image_size, self._theme.text_color)

    def _get_position(self, group: int = None, week_day: WeekDay = None,
        ...
        if week_day is not None:
            y_position += self._header_height + \
                          self._schedule_metrics.days_offsets[week_day] * \
                          self._cell_size[1] + \
                          self._schedule_metrics.week_days.index(week_day) * \
                          self._separator_width
    ...
    def _render_sidebar(self) -> None:
        self._render_sidebar_week_day(-1, (0, 0), 2)
        self._render_sidebar_entry_numbers(0, (0, 0), 1, '')
        self._render_sidebar_entry_numbers(1, (0, self._cell_size[1]), 1, '№')

        for day_number, week_day in enumerate(self._schedule_metrics.week_days):
            position = self._get_position(week_day=week_day)
            self._render_sidebar_week_day(
                day_number, position, self._schedule_metrics[week_day],
                week_day)
            self._render_sidebar_entry_numbers(
                self._schedule_metrics.days_offsets[week_day], position,
                self._schedule_metrics[week_day])

    def _render_sidebar_week_day(
        ...
        text: str = week_day.translate() if week_day is not None else ''
        if text:
    ...
    def _render_header(self, groups: List[Group]) -> None:
    ...
    def _render_schedule(self, schedule: Schedule) -> None:
        for group_number, group in enumerate(schedule.groups):
            for week_day in self._schedule_metrics.week_days:
                for entry_number in range(self._schedule_metrics[week_day]):
                    position = self._get_position(group_number, week_day,
                                                  entry_number)
                    row_number = self._schedule_metrics.days_offsets[
                                     week_day] + entry_number
    ...
    def _render_group_schedule(self, group_schedule: GroupSchedule,
        ...
        for week_day in group_schedule.days:
            day_schedule = group_schedule[week_day]
            self._render_day_schedule(day_schedule, group_number, week_day)

    def _render_day_schedule(self, day_schedule: DaySchedule,
                             group_number: int, week_day: WeekDay) -> None:
        for entry_number, entry in enumerate(day_schedule):
            position = self._get_position(group_number, week_day, entry_number)
            if entry is None:
